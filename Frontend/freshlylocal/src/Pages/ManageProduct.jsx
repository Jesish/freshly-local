...
// C:\Users\CHME\Desktop\freshly-local\frontend\src\components\ManageProducts.jsx
...
const ManageProducts = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      const payload = newProduct.image
        ? (() => {
      ...
      const response = await axios.post(
        ...
        payload,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
            ...(newProduct.image && { "Content-Type": "multipart/form-data" }),
          },
        }
      );
      setProducts((prev) => [...prev, response.data.product]);
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            ...
            className="flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg hover:from-green-700 hover:to-green-800 transition-all"
          >
            <Plus className="w-4 h-4 mr-2" /> Add Product
          </button>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          ...
          {products.map((product) => (
            <div
              key={product._id}
              className="grid grid-cols-6 gap-4 p-4 border-t hover:bg-gray-50 transition-colors"
            >
              <div className="flex items-center gap-2">
                {product.image && (
                  <img
